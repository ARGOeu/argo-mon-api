...
public interface DowntimeMapper {
    ...
    @Mapping(target = "downtime", ignore = true)
    DowntimeServiceEndpoint dtoToDowntimeService(
            DowntimeServiceEndpointRequest dto
    );
    ...
    List<DowntimeResponse> downtimesToDtos(
            List<Downtime> entities
    );


    @Mapping(source = "service.hostname", target = "hostname")
    @Mapping(source = "service.service", target = "service")
    DailyDowntimeEndpointResponse toDailyDowntimeEndpoint(
